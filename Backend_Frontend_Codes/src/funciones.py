...
def separarCaracteresLinea(linea):
  l1 = ''
  for caracter in linea:
    #mira si es letra de A(65) a Z(90) o a(97) a z(122) o 0(48) a 9(57) o espacio(32) o guionbajo(95)
    n = ord(caracter)
    if( (n>=65 and n<=90) or (n>=97 and n<=122) or (n>=48 and n<=57) or (n==32) or (n==95)):
      l1+=caracter
      #print(caracter)
      #print('letra')
    else:
...
#detecta los comentarios grandes los que empiezan en tres comillas dobles o simples
def detectarComentariosGrandes(lista):
  lista2=[]
  i=0
  b1=True
  b2=True
  while(i<len(lista)):
    if(i<len(lista)-2):
      if(lista[i]=="'" and lista[i+1]=="'" and lista[i+2]=="'"):
        i=i+2
        if(b1):
          b1=False
        elif(b1==False):
...
def concatenarTokensEnUnaLista(listasLineas):
  ...
  listaT = detectarComentariosGrandes(listaT)

  return listaT
############ELIMINAR NOMBRE DE VARIABLES#######
def palabras_no_contenidas(lista1, lista2):
    """
    Recibe dos listas de palabras y devuelve una lista con las palabras de la
    primera lista que no están en la segunda lista.
    """
    resultado = []
...
#metodo para eliminar nombre de variables\
#entrada lista de listas de tokens, ejm: [[a,=,2],[b,=,2]]
def eliminarNombreVariables(listas):
  nombre_variables=[]
  j=0
  for lista in listas:
    for i in range(0,len(lista)):
      if(lista[i]=='=' and i>=1 and i<len(lista)-1):
        bandera=False
        if(lista[i-1]=='!' or lista[i-1]=='>' or lista[i-1]=='<' or lista[i-1]=='='):
          bandera=True
        if(lista[i+1]=='!' or lista[i+1]=='>' or lista[i+1]=='<' or lista[i+1]=='='):
          bandera=True
        if(bandera==False):
          nombre_variables.append(lista[i-1])
    lista_sin_variables = palabras_no_contenidas(lista, nombre_variables)
    listas[j]=lista_sin_variables
    #print(listas[j])
    j=j+1
...
def tokensNormalizadosCodigo(ruta):
  ...
  lista=eliminarNombreVariables(lista)
...
def calcularDisimilitudVectorial2(c1,c2):
  #paso 1 normalizar las cadenas
  cadNorm = funciones2.normalizacionCodigosTexto(c1,c2)
  #paso 2 creacion del diccionario de tokens
  dic = creacionDiccionario(cadNorm)
  #paso 3 creacion bolsa de palabras
  bP = creacionBolsaPalabras(dic, cadNorm)
...
def definiendoRutaExacta(nombres,raiz):
  dir = []
  for ruta in nombres:
    direccion = os.path.join(raiz,ruta)
    #print(direccion)
...
def calcularMatrizSimlitud(nombresArchivos,raiz,tipo,url_resultados):
  ...
  funciones2.guardarCSV(filas,nombresArchivos,url_resultados)
